...
Modelos de Lenguaje de Gran Escala (\englishText{LLMs, Large Language Models}). Si bien los \englishText{LLMs} tienen 
...
interoperabilidad. Para ello, se propone un sistema que integra las siguientes etapas:
...
\boldparagraph{Procesamiento y normalización clínica}{
    Identificación de conceptos médicos y su vinculación con terminologías estándar internacionales 
    (\englishText{SNOMED CT, CIE-10-ES})\citeText{snomed2026} \citeText{sanidad2026} para garantizar un lenguaje común.
}
...
\boldparagraph{Salida estructurada (\englishText{JSON}/\englishText{FHIR})}{
    ...
    legibles por máquinas. Se prioriza el estándar \englishText{HL7 FHIR} \cite{sanidad2023}
    (\englishText{Fast Healthcare Interoperability Resources}) para 
...
la redacción de informes consume entre el 25\% y el 40\% del tiempo clínico\citeText{clynch2015documentation},
...
a partir de dictados médicos en español. El sistema deberá asegurar la interoperabilidad
...
\begin{itemize}
    ...
    \item Lograr que al menos el 80\,\% de los informes generados sean aceptados por el personal médico o un agente de IA
...
\section{Estructura del documento}\label{section:estructura_documento}

El resto de la memoria se organiza como sigue. El
Capítulo~\ref{chapter:marco_teorico_y_referencial} revisa el estado del arte en
\englishText{ASR}, \englishText{PLN} y \englishText{LLMs} aplicados a la documentación clínica, así
como el marco teórico y conceptual en el que se apoya el sistema. El
Capítulo~\ref{chapter:desarrollo} describe la metodología y la arquitectura del sistema desarrollado,
detallando cada etapa del \textit{pipeline} y su implementación. El Capítulo~\ref{chapter:evaluacion}
presenta el diseño experimental y las métricas empleadas para evaluar tanto los módulos individuales
como el sistema \englishText{end-to-end}. El Capítulo~\ref{chapter:resultados} expone los
resultados obtenidos y los discute a la luz de los objetivos planteados en esta introducción. El
Capítulo~\ref{chapter:conclusiones} resume las conclusiones generales del trabajo y propone líneas de
trabajo futuro. Finalmente, los apéndices recogen la planificación y gestión del proyecto, un análisis
de innovación y modelo de negocio, los \textit{prompts} empleados con modelos de lenguaje como agentes
auxiliares, el manual de usuario del sistema y los resultados completos de la evaluación
\englishText{end-to-end}.
