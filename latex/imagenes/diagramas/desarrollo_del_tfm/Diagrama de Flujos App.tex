\begin{figure}[htbp]
    ...
    \end{adjustbox}%
    \shorthandon{<>}
    \caption{Diagrama de secuencia del flujo de transcripción en tiempo real: captura de audio, transcripción \englishText{ASR} y enriquecimiento semántico por fragmento.}
...
%         \node[box] (G) at (2.3,0) {Gateway\\\mdseries (G :8000)};
%         \node[box] (A) at (4.8,0) {ASR Service\\\mdseries (A :8001)};
%         \node[box] (L) at (7.2,0) {LLM\\\mdseries (Ollama) (L)};

%         % --- Líneas de vida ---
%         \pgfmathsetmacro{\lifelineDepth}{-6.0}
%         \foreach \n in {M,G,A,L} {
%             \draw[lifeline] (\n.south) -- (\n.center |- 0,\lifelineDepth);
%         }
%         \node[box, font=\tiny\bfseries] (M bot) at (M.center |- 0,\lifelineDepth) {App móvil};
...
%         % --- Cierre de sesión ---
%         \draw[call] ($(M.south)+(0,-0.4)$) -- ($(G.south)+(0,-0.4)$)
%             node[msg] {JSON session\_end};

%         \draw[call]
%             ($(G.south)+(0,-0.8)$) -- ++(0.45,0) -- ++(0,-0.35) -- ($(G.south)+(0,-1.15)$)
%             node[note, right, xshift=0.15cm] {Espera transcripciones\\pendientes};

%         \draw[call] ($(G.south)+(0,-1.65)$) -- ($(A.south)+(0,-1.65)$)
%             node[msg] {POST /finalize-session/[id]};

%         \begin{scope}[on background layer]
%             \coordinate (optReportsNW) at (3.6,-1.8);
%             \coordinate (optReportsSE) at (5.95,-2.7);
%             \node[
%                 draw=blue!70!black,
%                 fill=blue!8,
%                 fill opacity=0.25,
...
%         \draw[call]
%             ($(A.south)+(0,-2.05)$) -- ++(0.45,0) -- ++(0,-0.35) -- ($(A.south)+(0,-2.4)$)
%             node[note, right, xshift=0.15cm] {Escribe \texttt{Reporte.txt}\\en disco};

%         \draw[resp] ($(M.south)+(0,-2.9)$) -- ($(G.south)+(0,-2.9)$)
%             node[msg] {WS session\_end};

%         % --- Informe LLM ---
%         \draw[call] ($(G.south)+(0,-3.4)$) -- ($(A.south)+(0,-3.4)$)
%             node[msg] {POST /generate-report/[id]};
%         \draw[call] ($(A.south)+(0,-3.9)$) -- ($(L.south)+(0,-3.9)$)
%             node[msg] {Lee Reporte.txt, genera informe};
%         \draw[resp] ($(L.south)+(0,-4.4)$) -- ($(A.south)+(0,-4.4)$)
%             node[msg] {Informe.txt};

%         \draw[resp] ($(G.south)+(0,-4.9)$) -- ($(A.south)+(0,-4.9)$)
%             node[msg] {JSON report};
%         \draw[resp] ($(M.south)+(0,-5.4)$) -- ($(G.south)+(0,-5.4)$)
